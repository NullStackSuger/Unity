...
public class OnCollisionEnterHandler : AEvent<OnCollisionEnter>
{
    ...
    {
        foreach (GameObject child in message.a.Foreach())
        {
            foreach (Component component in child.Components)
            {
                EntitySystem.Instance.CollisionEnter(component);

                foreach (Component componentB in message.b.Components)
                {
                    EntitySystem.Instance.CollisionEnter(component, componentB);
                }
            }
        }
...
public class OnCollisionStayHandler : AEvent<OnCollisionStay>
{
    protected override async ETTask Run(OnCollisionStay message)
    {
        foreach (GameObject child in message.a.Foreach())
        {
            foreach (Component component in child.Components)
            {
                EntitySystem.Instance.CollisionStay(component);

                foreach (Component componentB in message.b.Components)
                {
                    EntitySystem.Instance.CollisionStay(component, componentB);
                }
            }
        }
...
public class OnCollisionExitHandler : AEvent<OnCollisionExit>
{
    protected override async ETTask Run(OnCollisionExit message)
    {
        foreach (GameObject child in message.a.Foreach())
        {
            foreach (Component component in child.Components)
            {
                EntitySystem.Instance.CollisionExit(component);

                foreach (Component componentB in message.b.Components)
                {
                    EntitySystem.Instance.CollisionExit(component, componentB);
                }
            }
        }
